...
def Parse_Json():
    ...
    with codecs.open((CDF_Path+"\Single Coil_1.30\cdf\device.jso"),encoding='utf-8') as data_file:
        ...
        with codecs.open((CDF_Path+"\Single Coil_1.30\cdf\help-en-GB.jso"), encoding='utf-8') as data_file:
    ...
    #        
        ...
        writer_1.writerow(["Group_Code", "Menu_Code", "Group_Name", "Name", "PNU", "Default_Value", "Minimum_Value", "Maximum_Value", "Unit", "Scaling", "Datatype", "Enum", "Read_Only", "Help", "Decimals", "Variable_Name", "Visibility_Index", "Visibility_Rule", "Alarm_Parameter_Idx", "Unique_ID", "ItemType", "CopyType", "Enum_Vis_List_Idx"])       
        
        
        Parameter_File = Global_Scripts_Path+"\Danfoss.Single Coil_Parameters.csv"
        ...
        Search_file = Global_Scripts_Path+"\Danfoss.Single Coil_Search.csv"
...
#         writer_3.writerow(["", Alarm_Name, Alarm_Menu_Code])      
    ...
    for i in range (0, Main_Items_Count):
        ...
        #                                 test.log (Visibility_Idx)
                                    ...
                                    ####VISIBILITY####
                                    with open((CDF_Path+r"\Single Coil_1.30\cdf\visib.js")) as visibility_file:
                                        vis = visibility_file.readlines()
                                        vis = str(vis)
                                        array = []
                                        for x in vis.split('\\n'):
        ...
        #                                 Unit_Value= Unit_List[i]["Text"]
                                    ...
                                    writer_1.writerow(["", Parameter_Menu_Code, SG+"-"+SG_2+"-"+SG_3 , Parameter_Name, PNU, Default_Value, Minimum_Val, Maximum_Val, Unit_Value, Datatype_Scaling, Data_Type, Enum_Value, Read_Only, Help_Info, Decimals, Items_Unique_Idx, Visibility_Idx, Visibility_Rule, "", Unique_ID, ItemType, CopyType, Enum_Vis_List])
...
#                                 ItemType = AlarmsValuesItemIdx[av]["ItemType"]
                            ...
                            writer_1.writerow(["", Parameter_Menu_Code, SG+"-"+SG_2 , Parameter_Name, PNU, Default_Value, Minimum_Val, Maximum_Val, Unit_Value, Datatype_Scaling, Data_Type, Enum_Value, Read_Only, Help_Info, Decimals, Items_Unique_Idx, Visibility_Idx, Visibility_Rule, "", Unique_ID, ItemType, CopyType, Enum_Vis_List])
...
#                         Unit_Value= Unit_List[i]["Text"]
                    ...
                    writer_1.writerow(["", Parameter_Menu_Code, SG, Parameter_Name, PNU, Default_Value, Minimum_Val, Maximum_Val, Unit_Value, Datatype_Scaling, Data_Type, Enum_Value, Read_Only, Help_Info, Decimals, Items_Unique_Idx, Visibility_Idx, Visibility_Rule,"", Unique_ID, ItemType, CopyType, Enum_Vis_List])
...
def Moving_Files():
    ...
    dest_file = Global_Scripts_Path+'\Danfoss.Single Coil_DB.csv'                    
    shutil.copy (src_file, dest_file)
    
    PName_Dup_List=[]
    file = Global_Scripts_Path+"\Danfoss.Single Coil_DB.csv"   
...
def Group_Count(Occurences_List):
    file = Output_Global_Path+"\Single Coil_Parameters_Groups.csv"
    openfile = open(file,'wt') 
...
def Compare_DB():
    file = Output_Global_Path+"\Single Coil_Database_Comparison.csv"
    openfile = open(file,'wt') 
    writer = csv.writer(openfile)
    writer.writerow(["Label","Name", "VariableName", "Sql_Visib_Rule", "Squish_Visib_Rule", "Result", "Sql_Help", "Squish_Help", "Result"])
     
    Sql_file = Global_Scripts_Path+"\AK_CC_DB_Sql.csv"
    param = testData.dataset(Sql_file)
    for par in param:
#         Sql_Name = testData.field(par, 0) 
        Sql_VisibilityRule = testData.field(par, 24)
        Sql_VariableName = testData.field(par, 25)
        Sql_Help_Info = testData.field(par,19)
         
 
        Squish_file = Global_Scripts_Path+"\Danfoss.Single Coil_DB.csv"   
